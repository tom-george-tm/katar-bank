...
from agent.utils.docai_utils import (
    extract_entities,
    extract_form_fields,
...
async def process_document(
    ...
    processor_id = getattr(settings, env_key, None)
    if not processor_id:
        raise ValueError(
            f"Processor ID not configured in settings. Set the {env_key} environment variable or .env value."
        )
